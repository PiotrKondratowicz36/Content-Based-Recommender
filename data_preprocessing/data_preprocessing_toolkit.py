...
class DataPreprocessingToolkit(object):
    ...
    def add_weekend_stay(df):
        ...
        # Write your code here #
        ########################
        date_from, date_to = [x.date() for x in df['date_from']], [x.date() for x in df['date_to']]
        df["weekend_stay"] = (np.busday_count(date_from, date_to, "0000110") > 0)
    ...
    def aggregate_group_reservations(self, df):
        ...
        # Write your code here #
        ########################
        sum_columns = group_reservations.groupby("group_id")[self.sum_columns].apply(lambda x: x.sum())
        mean_columns = group_reservations.groupby("group_id")[self.mean_columns].apply(lambda x: x.mean())
        mode_columns = group_reservations.groupby("group_id")[self.mode_columns].agg(lambda x: x.value_counts().index[0])
        first_value_columns = group_reservations.groupby("group_id")[self.first_columns].first()
        group_reservations = pd.concat([sum_columns, mean_columns, mode_columns, first_value_columns], axis=1)
    ...
    def map_night_prices_to_room_segment_buckets(self, df):
        ...
        # Write your code here #
        ########################
        df["room_segment"] = df.groupby("room_group_id")["night_price"].transform(
            lambda x: self.map_value_to_bucket(x.mean(), self.room_segment_buckets))
        return df
